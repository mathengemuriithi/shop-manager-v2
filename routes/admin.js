...
router.get('/', (req, res) => {
  const data = readData();
  const totalProducts = data.products.filter(p => p.type === 'product').length;
  const totalServices = data.products.filter(p => p.type === 'service').length;
  const lowStock      = data.products.filter(p => p.type === 'product' && p.stock !== null && p.stock <= 3);
...
router.post('/products/edit/:id', upload.single('image'), (req, res) => {
  ...
  if (index === -1) { req.flash('error', 'Product not found.'); return res.redirect('/admin/products'); }
...
router.post('/categories/add', (req, res) => {
  ...
  if (!name || !name.trim()) { req.flash('error', 'Category name is required.'); return res.redirect('/admin/categories'); }

  const exists = data.categories.find(c => c.name.toLowerCase() === name.trim().toLowerCase());
  if (exists) { req.flash('error', 'Category already exists.'); return res.redirect('/admin/categories'); }
...
router.post('/categories/delete/:id', (req, res) => {
  ...
  if (!cat) { req.flash('error', 'Category not found.'); return res.redirect('/admin/categories'); }

  const inUse = data.products.some(p => p.categoryId === req.params.id);
  if (inUse) { req.flash('error', `Cannot delete "${cat.name}" — it has products assigned to it.`); return res.redirect('/admin/categories'); }
